...
FG-2 (add Reflex/PCL/DLSS_G to slInit featuresToLoad) — **CODED, awaiting Gate B build**, then FG-3 swapchain routing.
...
- [~] P3-2 **slInit features** — **CODED 2026-07-11, awaiting Gate B/C.** `SlBridge.buildPreferences` now loads `{DLSS, Reflex(3), PCL(4), DLSS_G(1000)}` (numFeatures=4); `logDlssSupport` checks all four via `slIsFeatureSupported` so Gate C confirms DLSS-G on the 4070S. Original: `featuresToLoad += kFeatureDLSS_G, kFeatureReflex, kFeaturePCL` (lazy slInit already precedes instance creation; proxies then add FG's extra device extensions/queues automatically).
